...
### TC-CLASS-05 Смена класса (free switching) — 🔴 P0
- [ ] Юзер с уже выбранным классом открывает `/class` (через «Сменить класс» на `/profile`)
- [ ] Заголовок страницы меняется на **«Сменить класс»**
- [ ] Подсказка вверху: «Перевыбери свой путь. Все ветви открыты»
- [ ] **Все 3 карточки доступны** (Django/DevOps больше НЕ залочены)
- [ ] На текущем классе:
  - бейдж **«✓ ТЕКУЩИЙ»** в правом верхнем углу
  - акцентная (оранжевая) обводка с glow
  - кнопка «✓ Текущий класс» серая, не активная (клик → редирект на /profile без запроса)
- [ ] На двух других карточках: кнопка «Сменить на этот класс»
- [ ] Клик на новый класс → `PATCH /api/profile/me/` с новым `class_role`
- [ ] Toast: **«Класс изменён! Новые силы пробуждаются...»**
- [ ] `window.confirm()` про intro **НЕ показывается** при повторной смене (только при первом выборе)
- [ ] Подсказка внизу: «Сменить класс можно в любой момент. Прогресс по миссиям сохраняется»
...
- [x] Анимация заполнения (CSS transition в XPBar)
